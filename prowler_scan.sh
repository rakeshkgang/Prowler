...
# Function to assume OIDC Role for GitHub Actions authentication
assume_oidc_role() {
    echo "Assuming OIDC Role for authentication..."
    ROLE_SESSION_CREDS=$(aws sts assume-role-with-web-identity --role-arn arn:aws:iam::924144197303:role/github-oidc-terraform-role-21 --role-session-name ProwlerRun --web-identity-token "$OIDC_TOKEN" --output json)
    AWS_ACCESS_KEY_ID=$(echo "$ROLE_SESSION_CREDS" | jq -r .Credentials.AccessKeyId)
    AWS_SECRET_ACCESS_KEY=$(echo "$ROLE_SESSION_CREDS" | jq -r .Credentials.SecretAccessKey)
    AWS_SESSION_TOKEN=$(echo "$ROLE_SESSION_CREDS" | jq -r .Credentials.SessionToken)
    export AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_SESSION_TOKEN
...
# Function to assume the ProwlerExecRole for assessments
assume_prowler_role() {
    echo "Assuming ProwlerExecRole for assessments..."
    ROLE_SESSION_CREDS=$(aws sts assume-role --role-arn arn:aws:iam::924144197303:role/ProwlerExecRole --role-session-name ProwlerRun --output json)
    AWS_ACCESS_KEY_ID=$(echo "$ROLE_SESSION_CREDS" | jq -r .Credentials.AccessKeyId)
    AWS_SECRET_ACCESS_KEY=$(echo "$ROLE_SESSION_CREDS" | jq -r .Credentials.SecretAccessKey)
    AWS_SESSION_TOKEN=$(echo "$ROLE_SESSION_CREDS" | jq -r .Credentials.SessionToken)
    export AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_SESSION_TOKEN
...
if [ "$REGION_LIST" == "allregions" ]; then
    for ACCOUNTID in $AWSACCOUNT_LIST; do
        test "$(jobs | grep Running | wc -l)" -ge $PARALLELISM && process_monitor || true
        {
            # Unset AWS Profile Variables
            unset_aws_environment
            echo -e "Assuming roles and assessing AWS Account: $ACCOUNTID with all AWS regions using Role: $IAM_CROSS_ACCOUNT_ROLE on $(date)"
            
            # Assume the OIDC role first to authenticate
            assume_oidc_role
            
            # Then assume the ProwlerExecRole to perform the Prowler scan
            assume_prowler_role

            # Run Prowler scan
            /usr/local/bin/prowler -R arn:aws:iam::$ACCOUNTID:role/$IAM_CROSS_ACCOUNT_ROLE -M csv json-ocsf html ${FINDING_OUTPUT:-} -T 1200 --verbose | tee output/stdout-$ACCOUNTID.txt 1>/dev/null
        } &
    done
fi
